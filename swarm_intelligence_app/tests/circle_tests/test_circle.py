...
class TestCircle:
    ...
    def test_organization(self, client):
        ...
        for token in self.tokens:
            ...
            circle_id_2 = self.get_circle_id(client, jwt_token, id2)
            self.post_circle_roles(client, jwt_token, circle_id_2)
            self.get_circle_roles(client, jwt_token, circle_id_2)
            role_id = self.get_role_id(client, jwt_token, circle_id_2)
            self.put_circle_subcircles(client, role_id, jwt_token)
            self.get_circle_members(client, circle_id_2, jwt_token)
            partner_id = self.get_partner_id(client, id2, jwt_token)
            self.put_circle_partner(client, partner_id, circle_id_2, jwt_token)
            self.delete_circle_partner(client, partner_id, circle_id_2,
                                       jwt_token)
    ...
    def post_circle_roles(self, client, token, circle_id_2):
        """
        Test if the post request gets executed.

        """
        assert client.post('/circles/' + circle_id_2 + '/roles', headers={
            'Authorization': 'Bearer ' + token},
                           data={'name': 'NewRole',
    ...
    def get_circle_roles(self, client, token, circle_id_2):
        """
        Test if the get request gets executed.

        """
        assert client.get('/circles/' + circle_id_2 + '/roles', headers={
            'Authorization': 'Bearer ' + token}).status == '200 OK'

    def get_role_id(self, client, token, circle_id_2):
        """
        Helper Method for getting the role Id

        """
        response = client.post('/circles/' + circle_id_2 + '/roles', headers={
            'Authorization': 'Bearer ' + token},
                           data={'name': 'NewRole',
                                 'purpose': 'This is a new Role added to a '
                                            'Circle.'})
    ...
    def put_circle_subcircles(self, client, role_id, token):
        ...
        assert client.put('/roles/' + role_id + '/circle', headers={
            'Authorization': 'Bearer ' + token},
                          data={'name': 'NewRole', 'purpose':
                                'This is a new Role added to a Circle.',
                                'strategy': 'NewStrategy'}).status == \
            '204 NO CONTENT'

    def get_circle_members(self, client, circle_id_2, token):
        """
        Test if the get request gets executed.

        """
        assert client.get('/circles/' + circle_id_2 + '/members', headers={
            'Authorization': 'Bearer ' + token}).status == '200 OK'
    ...
    def put_circle_partner(self, client, partner_id, circle_id_2, token):
        """
    ...
    def delete_circle_partner(self, client, partner_id, circle_id_2, token):
        ...
        """
        assert client.delete('/circles/' + circle_id_2 + '/members/' +
                             partner_id,
                             headers={'Authorization': 'Bearer ' + token}
